  ...
  return (
    <div className='relative w-[50%] rounded-lg overflow-hidden'>
      ...
      className={ ` flex mb-[2rem] transition-opacity duration-300 ${
        showThumbnails|| isHovered? "text-opacity-100" :"opacity-0"
      }`} >
         ...
         >
            {images.map((image,index)=>(
                <div key={index} className="min-w-full  " >
                    <img src = {image} alt={`Slide ${index+1}`} className='w-full'></img>
                 </div>
            ))}
        </motion.div>
      </div>
      ...
      <button
      onClick={prevSlide}
      className={`absolute left-4 transform -translate-y-1/2 transition-opacity duration-300
        ${
      ...
      <button
      onClick={nextSlide}
      className={`absolute right-4 transform -translate-y-1/2 transition-opacity duration-300
        ${
            isHovered?"opacity-100": "opacity-0"
        }
        `}
